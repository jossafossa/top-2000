...
export default class API {
  ...
  async getPlaylistTracks(id) {
    ...
    while (!foundAll) {
      let response = await this.spotify.getPlaylistTracks(id, {offset, limit});
  ...
  async update() {
    ...
    if (typeof states === "object" && "context" in states) {
      ...
      // album change
      if (states.item && states.item.album && states.item.album.id !== this.album) {
      ...
      // song change
      if ("item" in states && "id" in states.item && states.item.id !== this.track) {
